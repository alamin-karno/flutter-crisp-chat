...
class FlutterCrispChat {
  ...
  static Future<void> openCrispChat({required CrispConfig config}) {
    ...
    if (email != null && !email.isEmail) {
      throw ArgumentError.value(email, 'config.user.email', 'Invalid email format provided.');
    }
    ...
    if (url != null && !url.isUrl) {
      throw ArgumentError.value(url, 'config.user.company.url', 'Invalid company URL format provided.');
    }
